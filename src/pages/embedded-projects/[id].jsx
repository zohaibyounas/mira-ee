...
const ProjectDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchServiceData = async () => {
      try {
        const res = await fetch(`/api/EmbedProjectData/${id}?language=${language || 'en'}`);
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchServiceData = async () => {
      try {
        const res = await fetch(`/api/EmbedAllProjects/${id}?language=${language || 'en'}`);
        if (!res.ok) {
          throw new Error('Failed to fetch data');
        }
        const data = await res.json();
          ...
          <div className="onovo-page-navigation">
            <div className="onovo-page-navigation-content">
              {prev_id != 0 && prev_id != undefined && (
                <Link
                  href={`/embedded-projects/${prev_id}`}
              ...
              <Link href="/embedded-projects" className="page-navigation__posts">
                <i className="fas fa-th" />
              </Link>
              {next_id != 0 && next_id != undefined && (
                <Link
                  href={`/embedded-projects/${next_id}`}
