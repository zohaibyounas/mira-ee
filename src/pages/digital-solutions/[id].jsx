...
const ServiceDetail = () => {
  ...
  useEffect(() => {
    if (!id) return; // Avoid running the fetch when id is not avail
    
    const fetchServiceData = async () => {
      try {
        const res = await fetch(`/api/DigiServiceData/${id}?language=${language || 'en'}`);
        if (!res.ok) {
  ...
  useEffect(() => {
    ...
    const fetchServiceData = async () => {
      try {
        const res = await fetch(`/api/DigiAllServices/${id}?language=${language || 'en'}`);
